...
def parse_args() -> argparse.Namespace:
    ...
    # MMHCL+ performance (MMHCL_Plus_Optimization_Report.tex)
    parser.add_argument(
        "--amp_mode",
        type=str,
        default="off",
        choices=["off", "bf16", "fp16"],
        help="Mixed precision for main_mmhcl_plus: bf16 (Tensor Cores, no GradScaler), "
        "fp16 (uses GradScaler), or off (FP32 baseline).",
    )
    parser.add_argument(
        "--grad_clip_norm",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="1 = torch.compile(ExpandedProjector, mode=reduce-overhead); requires PyTorch 2.x.",
    )
    parser.add_argument(
        "--wema_ann_backend",
        type=str,
        default="auto",
        choices=["auto", "torch", "faiss"],
        help="Neighbor retrieval for W_ema soft-topology Jaccard: faiss (GPU if available), "
        "torch (cosine proxy only), auto (faiss when installed).",
    )
    parser.add_argument(
        "--wema_ann_k",
        type=int,
        default=32,
        help="Number of ANN neighbors per node for purification Jaccard (when backend is faiss).",
    )
