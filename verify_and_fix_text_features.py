...
from preprocess_helpers import clip_text_embeddings, iter_gzip_jsonlines
...
for name, path in files_to_check.items():
    ...
    if name.endswith(".json"):
        ...
        print(
            f"  [OK]  {name:<25}  {size_mb:6.1f} MB  |  {n_users:,} users  {n_ints:,} interactions"
        )
    else:
        arr = np.load(path)
        n_nz = np.count_nonzero(arr.sum(axis=1))
        print(
            f"  [OK]  {name:<25}  {size_mb:6.1f} MB  |  shape={arr.shape}  non-zero={n_nz:,}"
        )
