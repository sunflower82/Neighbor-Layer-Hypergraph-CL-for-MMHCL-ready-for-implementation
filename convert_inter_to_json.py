...
def convert_inter_to_json(inter_file, output_dir, dataset_name, core=5):
    ...
    with open(inter_file, encoding="utf-8") as f:
        # Skip header
    ...
    if core > 0:
        ...
        val_data = {
            uid: [item for item in items if item in valid_items]
            for uid, items in val_data.items()
            if uid in train_data
        }
        test_data = {
            uid: [item for item in items if item in valid_items]
            for uid, items in test_data.items()
            if uid in train_data
        }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert .inter file to MMHCL JSON format"
    )
    parser.add_argument(
        "--inter_file", type=str, required=True, help="Path to .inter file"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument("--dataset", type=str, default="Clothing", help="Dataset name")
    parser.add_argument(
        "--core", type=int, default=5, help="Core filtering value (default: 5)"
    )
